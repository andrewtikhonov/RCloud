 ...
 */
public class ShellConsoleView extends DynamicView {

    private ShellConsole shellconsole;
    private SearchPanel searchPanel = null;

    public ShellConsoleView(String title, Icon icon, int id, RGui rgui) {
        super(title, icon, new JPanel(), id);

        shellconsole = new ShellConsole(rgui, id);
        searchPanel = new SearchPanel(shellconsole);

        ((JPanel) getComponent()).setLayout(new BorderLayout());
        ((JPanel) getComponent()).add(new JScrollPane(shellconsole), BorderLayout.CENTER);
        ((JPanel) getComponent()).add(searchPanel, BorderLayout.SOUTH);
    }

    public void dispose() {
        shellconsole.dispose();
    }

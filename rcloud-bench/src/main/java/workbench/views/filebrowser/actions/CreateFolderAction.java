...
public class CreateFolderAction extends AbstractAction {
    ...
    private static Logger log = LoggerFactory.getLogger(CreateFolderAction.class);

    public CreateFolderAction(RGui rgui, FileBrowserPanel panel, String name){
        super(name);
        this.rgui  = rgui;
        this.panel = panel;
    }
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        new Thread(new Runnable(){
            public void run(){
                ...
                String folderName = (String) JOptionPaneExt.showInputDialog(panel,
                                    "Create a new folder:",
                                    "Create Folder", JOptionPane.PLAIN_MESSAGE,
                                    null, null, "untitled folder");
                ...
                for (FileNode node : toCreateIn) {
                    try {
                        if (node.isDirectory()) {
                            String folder = node.getPath() + FileNode.separator + folderName;

                            rgui.obtainR().createRandomAccessDir(folder);


                        } else  {
                            String destpath = node.getPath();
                            destpath = destpath.substring(0, destpath.lastIndexOf(FileNode.separator));
                            destpath = destpath + FileNode.separator + folderName;

                            rgui.obtainR().createRandomAccessDir(destpath);
                        }
                    } catch (Exception ex) {
                        //JOptionPaneExt.showExceptionDialog(panel.getParent(), ex);
                        log.error("Problem during folder creation",ex);
                        break;
                    }
                }
            }

...
public class LoadImageAction extends AbstractAction {
    ...
    final private static Logger log = LoggerFactory.getLogger(LoadImageAction.class);

    private RGui rgui;

    public LoadImageAction(RGui rgui, String name){
        super(name);
        this.rgui = rgui;
    }
        ...
        new Thread(new Runnable() {
            public void run() {
                Operation workspaceOp =
                        rgui.getOpManager().createOperation("Loading Workspace ", true);
                try {
                    workspaceOp.startOperation();

                    ServerRuntimeImpl.loadimage(rgui.getRuntime().getSession());
